<template>
  <div class="pay">
    ...
    <div class="address">
      ...
      <span class="mobile">1367890123</span>
      </div>
...
<script>
import { getAddress, handelPay } from '@/api/pay'
...
export default {
  ...
  methods: {
    ...
    async getOrderList () {
      // 不同的mode，请求数据的方不同
      if (this.mode === 'cart') {
        const { data: { order, personal } } = await getOrderData(this.mode, { cartIds: this.cartIds })
        this.order = order
        this.personal = personal
      }
      if (this.mode === 'buyNow') {
        const { data: { order, personal } } = await getOrderData(this.mode, {
